...
if "%BUILDER%"=="VS" (
    ...
    if exist addons\%ADDON_NAME%\scripts\ci\install.sh (
      ...
      set OF_ROOT=
      call %MSYS2_PATH%\usr\bin\bash -lc "addons/%ADDON_NAME%/scripts/ci/install.sh"
      set OF_ROOT=%TMP_OF_ROOT%
      set TMP_OF_ROOT=
    )

    rem Windows batch install script, if it exists.
    if exist addons\%ADDON_NAME%\scripts\ci\install.bat call addons\%ADDON_NAME%\scripts\ci\install.bat
    rem Windows Powershell script, if it exists.
    if exist addons\%ADDON_NAME%\scripts\ci\install.ps call addons\%ADDON_NAME%\scripts\ci\install.ps

    rem Install any addon-platform-specific dependencies.
    if exist addons\%ADDON_NAME%\scripts\ci\vs\install.sh (
       rem Temporarily unset the OF_ROOT, which is a windows path and rely on the makefiles to determine a posix OF_ROOT.
       set TMP_OF_ROOT=%OF_ROOT%
       set OF_ROOT=
       call %MSYS2_PATH%\usr\bin\bash -lc "addons/%ADDON_NAME%/scripts/ci/vs/install.sh"
       set OF_ROOT=%TMP_OF_ROOT%
       set TMP_OF_ROOT=
    )

    rem Windows batch install script, if it exists.
    if exist addons\%ADDON_NAME%\scripts\ci\vs\install.bat call addons\%ADDON_NAME%\scripts\ci\vs\install.bat
    rem Windows Powershell script, if it exists.
    if exist addons\%ADDON_NAME%\scripts\ci\vs\install.ps call addons\%ADDON_NAME%\scripts\ci\vs\install.ps

    rem Generate VS Solutions for each example in the addon folder.
    for /D %%e in (addons\%ADDON_NAME%\example*) do (
...
if "%BUILDER%"=="MSYS2" (
    ...
    if exist addons\%ADDON_NAME%\scripts\ci\install.sh (
        ...
        set TMP_OF_ROOT=%OF_ROOT%
        set OF_ROOT=
        %MSYS2_PATH%\usr\bin\bash -lc "addons/%ADDON_NAME%/scripts/ci/install.sh"
        set OF_ROOT=%TMP_OF_ROOT%
        set TMP_OF_ROOT=
    )

    rem Install any addon-platform-specific dependencies.
    if exist addons\%ADDON_NAME%\scripts\ci\msys2\install.sh (
        rem Temporarily unset the OF_ROOT, which is a windows path and rely on the makefiles to determine a posix OF_ROOT.
        set TMP_OF_ROOT=%OF_ROOT%
        set OF_ROOT=
        %MSYS2_PATH%\usr\bin\bash -lc "addons/%ADDON_NAME%/scripts/ci/msys2/install.sh"
        set OF_ROOT=%TMP_OF_ROOT%
        set TMP_OF_ROOT=
    )
